...
const processedSessions = new Set(); // Track processed sessions to prevent duplicate emails
const activeSessions = new Map(); // Track active session data for disconnect handling
...
let io; // Socket.IO instance (declared at module scope so cleanup timers can reference it)
...
// Extract Socket.IO setup into a function
function setupSocketIO(io) {
  ...
  io.on("connection", (socket) => {
    ...
    // Check for duplicate connections from same IP (prevent multiple connections per user)
    ...
    if (existingConnections.size >= 50) { // Allow max 50 connections per IP (increased for development)
        ...
        if (session) {
          ...
          try {
            ...
            if (storedMessages && storedMessages.length > 0) {
              ...
              // Restore OpenAI thread context if it exists in storage
              const sessionData = await storage.getSession(sessionId);
              if (sessionData && sessionData.threadId && !sessionThreads.has(sessionId)) {
                sessionThreads.set(sessionId, sessionData.threadId);
                console.log(`🧵 Restored OpenAI thread for session: ${sessionId} -> ${sessionData.threadId}`);
              }
            } else {
  ...
  socket.on("message", async (data) => {
    try {
      ...
      const validatedMessage = insertMessageSchema.parse(message);
        ...
        if (!activeSessions.has(currentSessionId)) {
          // Start a fresh in‑memory session with no hardcoded bot greeting.
          // The Assistant's own instructions should control how it greets the user.
          ...
          console.log("📝 Created new session without hardcoded greeting:", currentSessionId);
        }
        ...
        try {
          ...
          console.log("🔑 OpenAI API Key:", process.env.OPENAI_API_KEY ? "Set" : "Not set"); // 30 second timeout
          ...
          if (!threadId) {
            ...
            // Store thread ID in storage
            try {
              await storage.updateSession(currentSessionId, { threadId: threadId });
              console.log(`🧵 Stored thread ID in storage: ${threadId}`);
            } catch (error) {
          ...
          if (isFirstMessage) {
            try {
              ...
              messageContent = generateContextMessage(validatedMessage.content, assistantConfig, sessionData?.messages || []);
              console.log("✅ Assistant configuration loaded successfully");
            } catch (error) {
              console.error("❌ Error loading assistant config, falling back to raw user message:", error);
              // If config loading fails, just send the plain user message.
              // The Assistant's own system instructions still define its behavior.
          ...
          if (error.message.includes("rate limit exceeded")) {
            const fallbackEmail = process.env.SUPPORT_EMAIL || process.env.EMAIL_FROM || "";
            const fallbackPhone = process.env.SUPPORT_PHONE || "";
            const contactSuffix =
              fallbackEmail || fallbackPhone
                ? ` Please contact us directly${fallbackEmail ? ` at ${fallbackEmail}` : ""}${fallbackEmail && fallbackPhone ? " or" : ""}${fallbackPhone ? ` on ${fallbackPhone}` : ""} for immediate assistance.`
                : "";
            errorMessage = "I'm currently experiencing high demand. Please try again in a few minutes." + contactSuffix;
          }
    ...
    socket.on("endSession", async (data) => {
      try {
        ...
        if (threadId) {
          try {
            // If OpenAI provides a method to archive/delete threads, we'd use it here
...
// Function to extract user email from messages (check both user and bot messages)
function extractUserEmailFromMessages(messages) {
  const emailRegex = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g;
  
  for (const message of messages) {
    if (message.content) { // Check all messages (user and bot)
...
async function sendChatTranscriptEmail(sessionData) {
  ...
  const recipientEmail = userEmail || process.env.EMAIL_TO || "Herringtonconsulting@gmail.com"; // Fallback to default
  ...
  // console.log("🚀 ~ sendChatTranscriptEmail ~ ipAddress:", ipAddress).join("\n---\n\n");
...
setInterval(() => {
  ...
  // Clean up stale user connections (connections that are no longer active)
